...
def kua_number_from_dob(d: date, gender: str) -> int:
    """
    Calculates the Kua number based on birth year and gender.
    Source: https://www.youtube.com/shorts/dh2HlrsRdsw
    """
    year = d.year
    year_sum = single_digit_sum(year)
    
    kua = 0
    if gender.lower() == "boy":
        kua = 11 - year_sum
        kua = single_digit_sum(kua)
        if kua == 0 or kua == 5:
            kua = 2
    else: # For girl or any
        kua = year_sum + 4
        kua = single_digit_sum(kua)
        if kua == 5:
            kua = 8
